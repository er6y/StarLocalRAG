...
**界面优化 (2024-12)**
- 将"近似深度"标签改为"检索数"，更准确地描述功能
- 统一"大模型"和"知识库"标签宽度为60dp，实现界面对齐美观
- 优化"检索数"和"重排数"文本框布局，使其更紧凑
- 新增"重排数"配置项，默认值为5，支持用户自定义设置
- 所有配置项均自动保存到ConfigManager配置文件
...
- 支持绕过知识库：当知识库选择为"无"或检索数设置为0时，直接调用大模型API
...
**功能描述**：
优化重排模型的线程配置，使其与系统配置管理器保持一致，提高模型推理性能和资源利用率。

**实现细节**：

1. **线程数配置统一**：
   - 修改RerankerModelHandler中的线程数设置
   - 从硬编码的`Math.min(4, Runtime.getRuntime().availableProcessors())`改为`ConfigManager.getThreads(context)`
   - 确保重排模型与其他ONNX模型使用相同的线程配置

2. **配置参数**：
   - 默认线程数：4（`DEFAULT_THREADS = 4`）
   - 用户可在设置界面中调整ONNX推理线程数
   - 配置会自动应用到所有ONNX模型，包括重排模型

**技术要点**：
- 统一线程配置管理，避免不同模型使用不同的线程设置
- 提供用户可配置的线程数选项，适应不同设备性能
- 确保配置的一致性和可维护性

#### 2.2.3 重排模型线程池架构优化

**功能描述**：
重构重排模型的线程池架构，解决嵌套线程池问题，统一异步处理模式，提高系统稳定性和性能。

**问题背景**：
原有架构中，RerankerModelManager的getModelAsync使用自己的线程池，而RagQaFragment在onModelReady回调中又尝试使用自己的executor，形成嵌套线程池，可能导致线程资源竞争和死锁问题。

**解决方案**：

1. **统一异步处理模式**：
   - 在RerankerModelManager中新增RerankerCallback接口，包含onRerankComplete、onRerankError、onRerankProgress方法
   - 新增rerankAsync方法，将重排逻辑完全封装在RerankerModelManager内部
   - 采用与EmbeddingModelManager相同的架构模式，确保一致性

2. **线程池整合**：
   - 重排工作完全在RerankerModelManager的线程池中执行
   - 使用mainHandler将结果回调到主线程
   - 消除RagQaFragment中的嵌套线程池使用

3. **代码简化**：
   - RagQaFragment的processWithReranker方法大幅简化
   - 统一错误处理和进度更新机制
   - 减少代码复杂度和维护成本
...
- 简化调用逻辑，提升系统稳定性
- 遵循单一职责原则，各组件职责清晰
- **推理超时优化**：将重排模型推理超时时间从30秒调整为2分钟，适应模拟器等慢速环境的性能需求
- **标记输入格式优化**：改进输入文本构建方式，从简单拼接格式 `query [SEP] document` 升级为标记输入格式 `[Q] query [SEP] [D] document [SEP]`，使用 `[Q]` 和 `[D]` 标记明确区分问题和文档部分，提高模型对内容关系的理解能力
...
**问题背景**：
重排模型在实际使用中可能出现"假死"或异常情况，需要通过详细的日志记录来定位问题根源。

**调试优化措施**：

1. **详细执行日志**：
   - **批处理级别日志**：记录每个批次的开始时间、文档数量和处理状态
   - **文档级别日志**：记录每个文档的处理开始时间、文本长度和预览内容
   - **分词过程日志**：记录分词开始时间、原始文本长度、截断状态和分词耗时
   - **模型推理日志**：记录推理开始时间、输入张量形状、推理耗时和输出处理时间
   - **张量操作日志**：记录输入张量创建、转换过程和清理状态

2. **性能监控**：
   - **时间戳记录**：在关键步骤添加毫秒级时间戳，精确测量各阶段耗时
   - **内存状态监控**：记录张量创建前后的内存使用情况
   - **异常检测**：捕获并详细记录推理过程中的异常信息
   - **资源清理验证**：确保每次推理后正确释放ONNX张量资源

3. **输入输出追踪**：
   - **输入文本分析**：记录输入文本的长度、字符预览和截断情况
   - **分词结果验证**：记录生成的token数量、attention mask和token type ids
   - **模型输出检查**：记录logits张量的维度、数据类型和计算得分
   - **批处理汇总**：记录整个批次的处理结果和总耗时

4. **故障诊断机制**：
   - **超时检测**：移除固定超时限制，改为无限等待机制，避免误判"假死"
   - **线程状态监控**：通过详细日志判断线程是否正常执行或真正卡死
   - **错误恢复**：在异常情况下提供详细的错误信息和恢复建议
   - **资源泄漏防护**：确保即使在异常情况下也能正确清理资源

**技术实现要点**：

1. **日志级别控制**：
   - 使用不同的日志级别（DEBUG、INFO、WARN、ERROR）分类记录信息
   - 在生产环境中可通过配置调整日志详细程度

2. **性能影响最小化**：
   - 日志记录操作尽量简化，避免影响模型推理性能
   - 使用异步日志记录机制，减少对主线程的影响
...
#### 3.16.1 RerankerModelHandler TokenizerManager 集成

**功能描述**：
将 RerankerModelHandler 中的简化 tokenizer 实现替换为项目统一的 TokenizerManager，提高 tokenization 的准确性和性能。

**实现细节**：

1. **TokenizerManager 集成**：
   - 在 RerankerModelHandler 中添加 `tokenizerManager` 成员变量
   - 在 `initialize` 方法中初始化 TokenizerManager 实例
   - 自动从模型文件目录加载对应的 HuggingFace tokenizer

2. **tokenization 方法重构**：
   - **主方法**: `tokenizeInput` - 智能选择使用 TokenizerManager 或回退方案
   - **优先方案**: `tokenizeWithTokenizerManager` - 使用 HuggingFace tokenizer 进行高质量 tokenization
   - **回退方案**: `tokenizeWithFallback` - 保留原有简化实现作为备用

3. **性能优化特性**：
   - 使用与 RAG、知识库构建相同的 HuggingFace tokenizer，确保一致性
   - 智能回退机制：TokenizerManager 不可用时自动使用简化实现
   - 详细的性能日志记录，包含 tokenization 耗时和 token 统计信息
   - 完善的异常处理和错误恢复机制

4. **技术改进**：
   - 消除了"重复造轮子"问题，复用项目现有的 tokenizer 基础设施
   - 提供渐进式升级路径，确保系统稳定性
   - 与其他模块保持技术栈一致性

**预期效果**：
- **准确性提升**：使用真实的 HuggingFace tokenizer 替代字符级简化映射
- **性能优化**：专业的 tokenization 算法，显著提升处理效率
- **维护性改善**：统一的 tokenizer 管理，减少重复代码和维护成本
...
为了进一步优化用户体验和界面美观度，根据用户反馈对RAG问答界面进行了重新设计。

#### 3.20.1 布局设计目标

**设计原则**：
- 紧凑型布局：充分利用水平空间，减少界面高度
- 功能就近：相关控件横向排列，提升操作便利性
- 视觉对齐：确保控件宽度和位置的视觉一致性

#### 3.20.2 具体布局调整

**1. 大模型与检索数横向排列**：
- 大模型下拉框宽度缩短，不再占满整行
- 检索数控件（标签+文本框）紧跟在大模型下拉框后面
- 使用ConstraintLayout约束实现精确对齐

**2. 知识库与重排数横向排列**：
- 知识库下拉框宽度与大模型下拉框对齐
- 重排数控件（标签+文本框）紧跟在知识库下拉框后面
- 保持与上一行相同的布局模式
...
**约束关系优化**：
- 使用`layout_constraintEnd_toStartOf`建立控件间的依赖关系
- 通过`layout_marginStart`和`layout_marginEnd`控制间距
- 确保所有控件在垂直方向上正确对齐
...
这次布局重新设计显著提升了界面的紧凑性和美观度，同时保持了功能的完整性和易用性。
